...
EditorManager::EditorManager(LevelAsset* levelRef)
	: m_LevelRef(levelRef)
{
	m_QuitButton = MakeRef<TextButtonWidget>(this, "Quit");
	m_QuitButton->setRelativePosition({ 0.f, 0.f });
	...
	m_SaveButton->setRelativePosition({ .42f, .51f });
	...
	m_ClearButton->setRelativePosition({ .53f, 0.f });
...
void EditorManager::loadLevel()
{
	...
	*m_LevelRef = *Assets::getLevelAsset(m_LevelSelector->getSelectedLevelName(), true);
...
void EditorManager::saveLevel()
{
	if (m_LevelRef)
	{
		AssetLoader::saveLevel(MakeRef<LevelAsset>(*m_LevelRef));
		m_LevelSelector->setEdited(false);
	}
}
...
void EditorManager::clearLevel()
{
	if (m_LevelRef)
	{
		...
		m_LevelSelector->setEdited(true);
	}
}

void EditorManager::createLevel()
{
	*m_LevelRef = LevelAsset();
